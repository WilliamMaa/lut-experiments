...
def main():
    ...
    try:
        ...
        print("错误: 找不到 'phase1_20_floats_dataset.pt'，请先运行 mnist_traindata.py")
        return

    # 加载基座模型
    try:
        base_model = SmallBaseModel(alpha=1.0).to(device)
        base_model.load_state_dict(torch.load('model_B_base.pth', map_location=device, weights_only=False))
        base_model.eval()
        print("成功加载基座模型 'model_B_base.pth'")
    except FileNotFoundError:
        print("错误: 找不到 'model_B_base.pth'")
        return
