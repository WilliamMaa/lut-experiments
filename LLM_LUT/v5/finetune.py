"""
v5 multi-layer fine-tuning with trainable hybrid LUT.
...
Usage:
    ...
    LD_LIBRARY_PATH="" HF_HUB_OFFLINE=1 CUDA_VISIBLE_DEVICES=1 python finetune.py \
        ...
        --output_dir results/finetune_v5_first
"""
...
def build_engine_for_layer(model, layer_id: int, group_count: int,
                           checkpoint_root: str) -> HybridPartialEngine:
    ...
    if not paths:
        raise ValueError(f"No v5 checkpoints for L{layer_id} G{group_count} in {ckpt_dir}")

    engine = HybridPartialEngine(model, layer_id, group_size=64)
    for p in paths:
...
def finetune(model, calib_loader, eval_loader, engines, epochs, lr, output_dir,
    ...
    down_projs = [model.model.layers[e.layer_id].mlp.down_proj for e in engines]
...
def main():
    ...
    parser.add_argument("--configs", required=True,
                        help="Comma-separated layer:count pairs, e.g. '21:8,22:8'")
    parser.add_argument("--checkpoint_root", default="../v5/outputs")
    ...
    parser.add_argument("--output_dir", default="results/finetune_v5_first")
    ...
    print("=" * 70)
    print("v5 Hybrid LUT Multi-Layer Fine-Tune")
    print("=" * 70)
    ...
    for layer_id, group_count in configs:
        engine = build_engine_for_layer(model, layer_id, group_count, args.checkpoint_root)
    ...
    # LUT storage: approximate from checkpoints
    total_bytes = 0
    for layer_id, group_count in configs:
