...
class HybridOProjEngine:
    """
    Partial o_proj replacement with trainable ensemble LUT.

    Similar to HybridPartialEngine, but operates on self_attn.o_proj.
    The output dimension is hidden_size, grouped the same way as down_proj.

    Args:
        model: LLM already on target device
        layer_id: decoder layer to patch
        group_size: output channels per group
        mode: "direct" -> LUT predicts o_proj(x) directly;
              "delta"  -> LUT predicts (o_proj(x) - x) and reconstruction is x + lut
    """

    def __init__(self, model, layer_id: int, group_size: int = 64, mode: str = "direct"):
        assert mode in ("direct", "delta"), f"Unknown o_proj mode: {mode}"
        self.model = model
        self.layer_id = layer_id
        self.group_size = group_size
        self.mode = mode
    ...
    def install(self):
        ...
        self._hook_handle = self.o_proj.register_forward_pre_hook(self._o_proj_pre_hook)
        ...
        print(f"[v5] Hybrid o_proj LUT installed: L{self.layer_id}, "
              f"groups={replaced_groups}, tables={len(replaced_groups)}, mode={self.mode}")
    ...
    def uninstall(self):
        ...
        self._cached_x = None
        self._cached_indices = {}
        print(f"[v5] Hybrid o_proj LUT removed: L{self.layer_id}")

    def _o_proj_pre_hook(self, module, input):
        """Cache o_proj input (attn_output) and precompute bin indices."""
    ...
    def _patched_forward(self, hidden: torch.Tensor) -> torch.Tensor:
        """Partial matmul + LUT fill for o_proj."""
        ...
        with torch.autocast(device_type=device.type, enabled=False):
            ...
            if not torch.isfinite(active_out).all():
                raise RuntimeError(f"[v5 L{self.layer_id} o_proj] active_out has NaN/Inf")
            ...
            for gid in self._replaced_groups:
                ...
                if self.mode == "delta":
                    g_start = gid * self.group_size
                    lut_out = lut_out + x_f32[:, :, g_start:g_start + self.group_size]
            ...
            if not torch.isfinite(lut_outputs_cat).all():
                raise RuntimeError(f"[v5 L{self.layer_id} o_proj] lut_outputs has NaN/Inf")
...
def load_o_proj_group_checkpoint(model, path: str) -> "HybridOProjEngine":
    """Load a single o_proj group checkpoint and return a fresh engine with that group."""
    import os
    ckpt = torch.load(path, map_location="cpu")
    layer_id = ckpt["layer_id"]
    engine = HybridOProjEngine(model, layer_id, group_size=ckpt["group_size"],
                               mode=ckpt.get("mode", "direct"))
    if ckpt["address_type"] == "2d":
        address = Address2D(
            addr_idx=ckpt["addr_idx"],
            addr_mean=ckpt["addr_mean"],
