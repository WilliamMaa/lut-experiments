...
**2026-09-26 进度**：命令 1（sp-1 × s=1.0）与命令 4（sp96 × s=1.6，
sp96 事故修复后重跑）已绿。修复有效性确认：`failed=0`、
`stale_resume_retries=0`（ours sp96 r1 格，
blkcluster_s8t40_20260926_152118.json）。
...
读数（对 §7 判决口径）：
- **P3 信号成立**：sp96 档出现完整第二级 churn——tier 满 →
  dropped_bytes 1.2–1.7GB/worker → recall 8,104 块 → b3 new_tok 从
  sp-1 的 22,444 涨到 61,799（重算回潮 ~2.75×），tier 稀缺性 regime
  真实出现；
- sp96 档 ours−b3 new_tok 差 **1.082×**（66,871 vs 61,799），在
  P2 的 ≤1.2× 口径内；hit 0.928 vs 0.938 基本持平；
- **注意**：sp-1 × s=1.0 的 b3 只有 1 个 rep（runs=1），重跑
  §5 命令 1 补齐到 2 reps（manifest 自动跳过已完成格）后再写结论。

**剩余指令（按顺序）**：§5 命令 2（sp-1 × s=1.6）→ 命令 3
（sp96 × s=1.0）→ §5a 两条 b3@s0 → 补跑命令 1 的 b3 r1。
