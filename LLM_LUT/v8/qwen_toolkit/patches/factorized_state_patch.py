#!/usr/bin/env python3
"""Factorized GDN recurrent state patch.

At runtime, every recurrent state written to the LinearAttentionLayer cache is
factorized via SVD (U * V^T) and reconstructed on read.  This tests whether a
low-rank state representation can survive end-to-end inference without changing
the update rule.
"""
...
class FactorizedStateContainer:
...
class FactorizedStatePatch(EvalPatch):
    """Apply low-rank SVD compression to GDN recurrent states at runtime."""
    ...
    def name(self) -> str:
        layers = f"_layers{sorted(self.layer_indices)}" if self.layer_indices else "_all"
        return f"factorized_state_r{self.rank}{layers}"
    ...
    def install(self, model: torch.nn.Module) -> None:
        ...
        def new_init(self, *args, **kwargs):
            ...
            if target_layers is None or self.layer_idx in target_layers:
                self.recurrent_states = FactorizedStateContainer(
                    rank, list(self.recurrent_states.keys()), self.dtype, self.device
                )

        def new_update(self, recurrent_states: torch.Tensor, state_idx: int = 0, **kwargs):
        ...
        def new_reset(self):
            for i in range(self.number_of_states):
                ...
                if isinstance(self.recurrent_states, FactorizedStateContainer):
        ...
        LinearAttentionLayer.update_recurrent_state = new_update
