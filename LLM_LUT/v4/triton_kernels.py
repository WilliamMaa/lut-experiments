    ...
    def _lut_fill_multi_group_kernel(
        ...
        for c in tl.range(0, GROUP_SIZE):
            ...
            table_idx = (b1 * stride_table_b1 +
                         b2 * stride_table_b2 +
                         c  * stride_table_c)
            lut_delta = tl.load(table_base + table_idx, mask=m_mask)
...
def triton_lut_fill(bin_idx, tables, normed_x, group_starts):
    """
...
# ---------------------------------------------------------------------------
def pytorch_lut_fill(bin_idx, tables, normed_x, group_starts):
...
def lut_fill(bin_idx, tables, normed_x, group_starts):
    """Unified entry: Triton if available, else PyTorch fallback."""
    if TRITON_AVAILABLE:
        try:
            return triton_lut_fill(bin_idx, tables, normed_x, group_starts)
        except Exception as e:
            print(f"[Triton] Kernel failed ({e}), falling back to PyTorch.")
            return pytorch_lut_fill(bin_idx, tables, normed_x, group_starts)
    else:
        return pytorch_lut_fill(bin_idx, tables, normed_x, group_starts)
