...
class KVMemory(nn.Module):
    ...
    """
    def __init__(self, feat_dim=16, mem_size=256, out_dim=20):
        super().__init__()
...
for i, (img, label) in pbar_eval:
    ...
    with torch.no_grad():
        ...
        # --- 方案 5: Ensemble 混合残差 ---
        w_ensemble = 0.4 * w_knn + 0.3 * w_rbf + 0.2 * w_vq + 0.1 * w_kv
        out_ensemble = model_B(img, dynamic_beta_weights=w_ensemble)
        if out_ensemble.argmax().item() == label:
            correct_ensemble += 1
...
print(f"5. Ensemble   混合残差   : {correct_ensemble / total * 100:.2f}%")
print("=" * 60)
